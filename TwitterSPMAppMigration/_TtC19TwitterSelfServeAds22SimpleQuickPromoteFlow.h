//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class MISSING_TYPE;

@interface _TtC19TwitterSelfServeAds22SimpleQuickPromoteFlow : NSObject
{
    MISSING_TYPE *context;
    MISSING_TYPE *modalSheet;
    MISSING_TYPE *navigationController;
    MISSING_TYPE *promotionCreator;
    MISSING_TYPE *didTapTweetAndBoost;
    MISSING_TYPE *terminateFlowBlock;
}

- (void).cxx_destruct;
- (id)init;
- (void)modalSheetViewControllerDidDismiss:(id)arg1 fromGesture:(_Bool)arg2;
- (double)preferredHeightForModalSheetViewController:(id)arg1 modalContentViewVisibleEdgeInsets:(struct UIEdgeInsets)arg2;

@end

